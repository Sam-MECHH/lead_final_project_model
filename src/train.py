...
def generate_encoded_tensors(x_set, type="train", chunk_size=2000):
    ...
    for img_paths, reports, indices in tqdm(train_loader):
        ...
        if len(img_buffer) >= chunk_size:
            torch.save(
                img_buffer,
                f"./data/features_chunks/image_patches_{type}/img_patches_chunk_{chunk_idx}.pt",
            )
            torch.save(
                text_buffer,
                f"./data/features_chunks/text_sequences_{type}/text_seq_chunk_{chunk_idx}.pt",
            )
...
def load_saved_images(chunk_idx, type="train"):
    ...
    for i in tqdm(range(chunk_idx + 1)):
        img_patches = torch.load(
            f"./data/features_chunks/image_patches_{type}/img_patches_chunk_{i}.pt",
            weights_only=True,
        )
        ret_array.extend(
            [img_patches[j].squeeze(0) for j in sorted(img_patches.keys())]
        )
...
def load_saved_texts(chunk_idx, type="train"):
    ...
    for i in tqdm(range(chunk_idx + 1)):
        text_sequence = torch.load(
            f"./data/features_chunks/text_sequences_{type}/text_seq_chunk_{i}.pt",
            weights_only=True,
        )
        ret_array.extend(
            [text_sequence[j].squeeze(0)[:256, :] for j in sorted(text_sequence.keys())]
        )

    return ret_array


def cross_attention_train_biovil(
    model,
    train_loader,
    val_loader,
    criterion,
    ...
    for epoch in range(epochs):  # Loop over the number of epochs
        ...
        for inputs_img, inputs_txt, labels in train_loader:
            ...
            correct += (
                ((torch.sigmoid(outputs) >= 0.5).float() == labels).sum().item()
...
if __name__ == "__main__":
    ...
    cross_att_X_train_txt_tensor = torch.stack(
        load_saved_texts(chunks_total_train, type="train")
    ).float()
    cross_att_y_train_tensor = torch.tensor(y_train.to_numpy(), dtype=torch.float32)
    cross_att_train_dataset = TensorDataset(
        cross_att_X_train_img_tensor,
        cross_att_X_train_txt_tensor,
        cross_att_y_train_tensor,
    )

    cross_att_X_test_img_tensor = torch.stack(
        load_saved_images(chunks_total_test, type="test")
    ).float()
    cross_att_X_test_txt_tensor = torch.stack(
        load_saved_texts(chunks_total_test, type="test")
    ).float()
    cross_att_y_test_tensor = torch.tensor(y_test.to_numpy(), dtype=torch.float32)
    cross_att_test_dataset = TensorDataset(
        cross_att_X_test_img_tensor,
        cross_att_X_test_txt_tensor,
        cross_att_y_test_tensor,
    )

    cross_att_train_loader = DataLoader(
        cross_att_train_dataset, batch_size=64, shuffle=True
    )
    cross_att_val_loader = DataLoader(
    ...
    with mlflow.start_run():
        ...
        optimizer = optim.AdamW(
            cross_att_model.parameters(), lr=1e-5, weight_decay=1e-2
        )
